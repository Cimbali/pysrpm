...
def test_template_specialisation():
    # Default
    assert get_specfile('BuildRequires') == ['findutils grep python3-devel', 'python%{python3_version}dist(setuptools), python%{python3_version}dist(wheel)']
    assert get_specfile('Provides') == ['python%{python3_version}dist(package)']

    # CLI / kwargs override
    assert get_specfile('BuildRequires', python_package='python-{name}') == ['findutils grep python3-devel', 'python-setuptools, python-wheel']
    ...
    try:
        assert get_specfile('BuildRequires', config=cfg) == ['findutils grep python3-devel', 'python-setuptools, python-wheel']
